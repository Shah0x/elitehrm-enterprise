...
file = file.replace(/✅/g, '[SUCCESS]');
file = file.replace(/🌱/g, '[INIT]');
file = file.replace(/📊/g, '[DATA]');
file = file.replace(/📝/g, '[LOG]');
file = file.replace(/🚀/g, '[READY]');
file = file.replace(/❌/g, '[ERROR]');
...
file = file.replace(/status: 'active'/g, "status: e.status || 'active'");
